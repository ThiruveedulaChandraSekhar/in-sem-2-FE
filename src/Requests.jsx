...
class Requests extends Component {
    ...
    loadInventoryAndRequests = () => {
        callApi("GET", "http://localhost:30083/back1/requests/inventory", "", (invResponse) => {
    ...
    loadRequests = () => {
        callApi("GET", "http://localhost:30083/back1/requests/readrequest", "", (res) => {
            ...
            joblist = joblist.map(r => {
                ...
                if (r.status === 'pending' && available > 0) {
                    ...
                    callApi("PUT", `http://localhost:30083/back1/requests/accept/${r.id}`, "", () => {});
                }
                return r;
            });
    ...
    saveRequest = () => {
        ...
        const url = this.state.id
            ? "http://localhost:30083/back1/requests/update"
            : "http://localhost:30083/back1/requests/insert";
    ...
    updateRequest = (id) => {
        callApi("GET", "http://localhost:30083/back1/requests/getrequest/" + id, "", (res) => {
            if (res.includes("401::")) { alert(res.split("::")[1]); return; }
    ...
    deleteRequest = (id) => {
        ...
        callApi("DELETE", "http://localhost:30083/back1/requests/delete/" + id, "", this.afterSave);
    }
